...
exports.getAccuracy = async (req, res) => {
  try {
    ...
    res.json({ accuracy: accuracy_percentage, samples: count, message: accuracy_percentage ? `${accuracy_percentage}% accuracy based on ${count} comparisons` : 'No comparisons available' });
  } catch (error) {
    res.status(500).json({ error: 'Internal server error' });
  }
};
